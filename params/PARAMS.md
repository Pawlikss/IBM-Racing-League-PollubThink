# Słownik parametrów `my_racer.py`

Wszystkie parametry konfigurowalne kontrolera. Definiowane w `PARAMS` dict w
[../my_racer.py](../my_racer.py), nadpisywane przez `params.json` (root) jeśli istnieje.

Optuna w [../optimize.py](../optimize.py) tunuje podzbiór tych parametrów w zależności
od MODE (env var) — patrz komentarze w optimize.py nad sekcją MODE.

---

## Struktura folderu `params/`
